...
基于ethernet的文件传输协议:(zn)
               client --  发起connected                                              --> server
                     <--                                                 "ok!"       --
                     --   ${file_name}-${file_size}:${file_data}                     -->            %eg:zhangn-3:123
                     --   client断开连接                                             -->
                     <--                                             server断开连接  --

server将接收到的文件按照file_name.设备+时间的形式存储在/home/Gateway_Control_System/storage
通过程序定期将storage文件同步到岸基云文档
